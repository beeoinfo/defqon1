...
export async function generateDefqon1Lineup() {
  ...
  const sourceUpdatedDate = getLatestUpdatedDate(data);
  const payload = {
  ...
  const artistCount = daysOut.reduce(
    (sum, day) => sum + day.stages.reduce((stageSum, stage) => stageSum + (stage.artists?.length || 0), 0),
    0
  );

  return {
    siteSlug: 'defqon1',
    versionLabel: formatLineupFilename(sourceUpdatedDate).replace(LINEUP_FILE_SUFFIX, ''),
    sourceUrl: 'https://www.q-dance.com/graphql/',
    sourceUpdatedAt: sourceUpdatedDate.toISOString(),
    sourceHash: null,
    payload,
    rawSource: raw,
    stats: {
