...
const STAGE_COLORS = {
  'STAGE MIRAGE': SITE_THEME.primary,
  'STAGE ALT F4': SITE_THEME.secondary,
  TECHNOBUS: SILVER,
...
export async function generateInsaneLineup(options = {}) {
  const source1Url = options.source1Url ?? process.env.INSANE_SOURCE1_URL ?? DEFAULT_SOURCE1_URL;
  const source1Har = options.source1Har ?? null;
  const source2Url = options.source2Url ?? process.env.INSANE_SOURCE2_URL ?? DEFAULT_SOURCE2_URL;
  const source2Har = options.source2Har ?? null;
  ...
  return {
    siteSlug: SITE_SLUG,
    versionLabel: formatLineupFilename(updatedAt).replace(LINEUP_FILE_SUFFIX, ''),
    sourceUrl: source2.url ?? source2Url,
